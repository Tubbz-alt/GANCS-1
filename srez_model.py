...
def _generator_model_with_scale(sess, features, labels, masks, channels, layer_output_skip=5,
    ...
    if num_dc_layers >= 0:
        ...
        for index_dc_layer in range(num_dc_layerss):
            ...
            corrected_real = tf.reshape(tf.real(corrected_complex), [FLAGS.batch_size, 256, 256, 1])
            corrected_imag = tf.reshape(tf.imag(corrected_complex), [FLAGS.batch_size, 256, 256, 1])
...
def create_model(sess, features, labels, masks, architecture='resnet'):
    ...
    with tf.variable_scope('gene') as scope:
        ...
        gene_output_i = gene_output_1
        for i in range(FLAGS.number_of_copies - 1):
            gene_output_i, _, gene_layers_i = function_generator(sess, gene_output_i, labels, masks, 1)
            scope.reuse_variables()

        gene_output_real = gene_output_i
        ...
        gene_moutput_i = gene_moutput_1
        for i in range(FLAGS.number_of_copies - 1):
            gene_moutput_i, _ , gene_mlayers_i = function_generator(sess, gene_moutput_i, labels, masks, 1)
            scope.reuse_variables()

        gene_moutput_real = gene_moutput_i
